...
static void testPutOk(const std::string &pvname, double v)
{
    ...
    testdbGetFieldEqual(pvname.c_str(), DBF_DOUBLE, v);
}
...
static void testPutOk(const std::string &pvname, const char *v)
{
    ...
    testdbGetFieldEqual(pvname.c_str(), DBF_STRING, v);
}
...
static void record_expect_invalid_INPB(const char *rec)
{
    ...
    testTodoBegin("UDF should be set for invalid LINK alarm");
    testdbGetFieldEqual(pv(rec, "UDF").c_str(), DBF_USHORT, 1);
    testTodoEnd();
}
...
static void test_broken_inp_link(const char *rec)
{
    ...
    testTodoBegin("Broken LINK bug: unused broken input should not invalidate result");
    setCalcAndProcess(rec, "A");
    record_expect_valid(rec, 10.0);
    testTodoEnd();

    testDiag("CASE %s: broken INPB but ternary selects A", rec);
    testTodoBegin("Broken LINK bug: unselected ternary branch should not invalidate result");
